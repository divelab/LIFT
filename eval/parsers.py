...
def test_solution(code_str, output_dir=None):
    ...
    try:
       ...
       # I want to print the exception here to debug if needed, but it causes issues with the current logging setup. For now, just return 0 on any exception.
        print(f"Exception during test execution: {e}")
        return 0


def extract_humaneval_prompt(prompt):
    in_docstring = False
    docstring_lines = []
    triple_quote = None
